...
//
// inputs:
// c  = constraints per light (0: no constraint, 1: sample from high, 2: sample from low, 3: Pinned)
  ...
  const float sh, const float sl)
{
  ...
  // determine how many lights have already been sampled from each distribution
  for (int i = 0; i < n; i++) {
    if (c[i] != 3)
      continue;
...
void GibbsSamplingGaussianMixturePrior(std::vector<float>& result, const std::vector<int>& c, const std::vector<float>& s,
  const int n, const int k, const float mh, const float ma,
  const float sparsity, map<int, pair<int, float>> rel,
  bool use_image_intensity, const float sh, const float sl)
{
  if (result.size() != n)
    result.resize(n);
  ...
  for (int i = 0; i < n; i++) {
    ...
    // check if we hit a light that controls another
    if (rel.count(i) > 0) {
      // update n stuff
      float dependentVal = current_sample * rel[i].second;
      result[rel[i].first] = dependentVal;

      if (dependentVal > mh - sh)
        sample_k++;

      n_sampled++;
      sample_mean = ((n_sampled - 1) * sample_mean + dependentVal) / n_sampled;
    }
  }
  ...
  // note that we aren't actually going to do anything with the dependent lights here
  // since that gets corrected at the end anyway
  if (use_image_intensity)
  {
    // our goal is to keep mean intensity as close to prescribed mean as possible. 
    // hence, sum_inv_s should be 1. (Because of the capping it will go a bit down,
